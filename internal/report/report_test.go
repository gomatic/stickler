...
func resultWith(diags []goyze.Diagnostic, errs []error) suite.Result {
	return suite.Result{Diagnostics: diags, Errors: errs}
}
...
func TestFormatHumanWritesDiagnosticsAndErrors(t *testing.T) {
	...
	require.NoError(t, report.Write(&buf, report.OutputHuman, res))
...
func TestFormatHumanReportsDiagnosticWriteError(t *testing.T) {
	...
	require.Error(t, report.Write(failWriter{}, report.OutputHuman, res))
}
...
func TestFormatJSONEncodesDiagnosticsAndErrors(t *testing.T) {
	...
	require.NoError(t, report.Write(&buf, report.OutputJSON, res))
...
func TestFormatJSONReportsWriteError(t *testing.T) {
	require.Error(t, report.Write(failWriter{}, report.OutputJSON, resultWith(nil, nil)))
}
...
func TestFormatGitHubEmitsAnnotationsByLevel(t *testing.T) {
	...
	require.NoError(t, report.Write(&buf, report.OutputGitHub, res))
...
func TestFormatGitHubReportsWriteError(t *testing.T) {
	res := resultWith([]goyze.Diagnostic{{Path: "a.go"}}, nil)
	require.Error(t, report.Write(failWriter{}, report.OutputGitHub, res))
}

func TestFormatRejectsUnknownFormat(t *testing.T) {
	err := report.Write(&bytes.Buffer{}, "nope", resultWith(nil, nil))
	require.Error(t, err)
	assert.True(t, errors.Is(err, constants.ErrUnknownOutput))
}
