...
func TestGhValueEscapesEveryCharacterThatCanBreakAnAnnotation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		assert.Equal(t, tc.want, escapeData(tc.in), "escapeData(%q): %s", tc.in, tc.why)
	}
}
...
func TestGhValuePropertyEscapingAlsoCoversTheDelimiters(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		assert.Equal(t, tc.want, escapeProperty(tc.in), "escapeProperty(%q): %s", tc.in, tc.why)
	}
}
...
func TestGhValueCannotEmitAWorkflowCommand(t *testing.T) {
	...
	for _, hostile := range []ghValue{
		...
		for _, got := range []string{escapeData(hostile), escapeProperty(hostile)} {
		...
		assert.NotContains(t, escapeProperty(hostile), ",", "a property delimiter survived")
		assert.NotContains(t, escapeProperty(hostile), ":", "a property terminator survived")
	}
}
