...
func resolveEnv(template []string) ([]EnvVar, func(), error) {
	...
	if err != nil {
		return nil, nothing, ErrRunnerFailed.With(err, "creating", "hermetic cache directory")
	}
...
func mentionsCache(template []string) bool {
	for _, entry := range template {
		if strings.Contains(entry, placeholderCache) {
			return true
		}
	}
	return false
...
func toEnv(template []string, dir cacheDir) []EnvVar {
	...
	for i, entry := range template {
		out[i] = EnvVar(strings.ReplaceAll(entry, placeholderCache, string(dir)))
	}
	return out
}
