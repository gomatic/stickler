...
// golangciCacheEnvVar is golangci-lint's cache selector, spelled out here on
// purpose: the test pins the WIRE name the subprocess must see, which a
// constant shared with the code under test could not do.
const golangciCacheEnvVar = "GOLANGCI_LINT_CACHE"

// hermeticRunner builds the single named default-spec runner over command.
func hermeticRunner(t *testing.T, command Command, name string) suite.Runner {
	t.Helper()
	runners, err := Build(command, Registry{Specs: config.DefaultRunnerSpecs()}, []string{name}, Context{})
...
func TestGolangciDefaultSpecRunsHermeticCache(t *testing.T) {
	want := assert.New(t)
	var seen []string
	command := func(_ context.Context, _ Name, env []EnvVar, _ ...Arg) ([]byte, error) {
		for _, entry := range env {
			value, ok := strings.CutPrefix(string(entry), golangciCacheEnvVar+"=")
			if !ok {
				continue
			}
	...
	runner := hermeticRunner(t, command, config.ToolGolangci)
	for range 2 {
...
func TestYzeDefaultSpecPassesNoEnv(t *testing.T) {
	...
	_, err := hermeticRunner(t, command, config.ToolYze).Run(context.Background(), ".")
	require.NoError(t, err)
	require.Len(t, got, 1)
	assert.Nil(t, got[0])
}
...
// created, the runner fails with constants.ErrRunnerFailed instead of running the tool
...
func TestResolveEnvCreationFailureSurfaces(t *testing.T) {
	...
	command := func(context.Context, Name, []EnvVar, ...Arg) ([]byte, error) {
		t.Fatal("the tool must not run without its hermetic cache")
		return nil, nil
	}

	_, err := hermeticRunner(t, command, config.ToolGolangci).Run(context.Background(), ".")
