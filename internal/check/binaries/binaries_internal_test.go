...
func TestBinariesReportsEveryNativeFormat(t *testing.T) {
	...
	diags, err := Runner{}.Run(context.Background(), suite.Root(dir))
...
func TestBinariesSurfacesGitFailure(t *testing.T) {
	...
	_, err := Runner{}.Run(context.Background(), suite.Root(t.TempDir()))

	assert.ErrorIs(t, err, constants.ErrExec)
}
...
func TestBinariesListSeam(t *testing.T) {
	...
	listTracked = func(context.Context, suite.Dir) ([]trackedPath, error) {
		return nil, constants.ErrExec.With(nil, "stub", "boom")
	}

	_, err := Runner{}.Run(context.Background(), "ignored")
	assert.ErrorIs(t, err, constants.ErrExec)
}
