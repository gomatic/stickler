...
// ConfigPath is the path of one stickler config layer (global, then repo).
type ConfigPath string

func LoadLayers(read func(path string) ([]byte, error), paths ...ConfigPath) ([]Config, error) {
	var layers []Config
...
func ConfigPaths(getenv func(string) string, home HomeDir, repoRoot RepoRoot) []ConfigPath {
	return []ConfigPath{globalConfigPath(getenv, home), ConfigPath(filepath.Join(string(repoRoot), ".stickler.yaml"))}
}
...
func globalConfigPath(getenv func(string) string, home HomeDir) ConfigPath {
	if xdg := getenv("XDG_CONFIG_HOME"); filepath.IsAbs(xdg) {
		return ConfigPath(filepath.Join(xdg, "stickler", "config.yaml"))
	}
	return ConfigPath(filepath.Join(string(home), ".config", "stickler", "config.yaml"))
}
